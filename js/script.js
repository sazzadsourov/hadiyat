...
function customCursor(options) {
    let settings = $.extend({
        targetClass: 'custom-cursor', // crea+te element with this class
        wrapper: $('body'), // jQuery
        speed: .1,
...
customCursor({
    ...
    onMove: function (data) {
        if (data.isInViewport) {
            ...
            if (data.isMoving) {
                if (data.isHover) {
                    gsap.to(data.cursor, { opacity: 1, scale: 1.2 });
                } else {
                    gsap.to(data.cursor, { opacity: .5, scale: .3 });
                }
            } else {
                if (data.isHover) {
                    gsap.to(data.cursor, { opacity: 0, scale: 1 });
                } else {
                    gsap.to(data.cursor, { opacity: .5, scale: .5 });
                }
            }
        } else {
            // out viewport
